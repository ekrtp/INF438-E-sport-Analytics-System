...
Un modèle de classification binaire a été développé pour prédire le résultat d'un match. La variable cible est \texttt{win} (1 = Victoire, 0 = Défaite).

\begin{table}[H]
\centering
\caption{Caractéristiques (Features) utilisées}
\label{tab:features}
\begin{tabular}{>{\centering\arraybackslash}m{3.5cm}>{\centering\arraybackslash}m{5cm}>{\centering\arraybackslash}m{3cm}}
\toprule
\textbf{Feature} & \textbf{Description} & \textbf{Catégorie} \\
\midrule
kills & Nombre d'éliminations & Combat \\
deaths & Nombre de morts & Combat \\
assists & Nombre d'assistances & Combat \\
gold\_per\_min & Or par minute & Économie \\
xp\_per\_min & Expérience par minute & Économie \\
hero\_damage & Dégâts aux héros & Dégâts \\
tower\_damage & Dégâts aux tours & Dégâts \\
last\_hits & Nombre de last hits & Progression \\
level & Niveau du joueur & Progression \\
duration\_minutes & Durée du match & Info Match \\
\bottomrule
\end{tabular}
\end{table}

L'algorithme \textbf{Random Forest Classifier} a été choisi avec les paramètres : numTrees=50, maxDepth=10, Train/Test Split=80\%/20\%.

Le code source complet du pipeline ML est disponible en \textbf{Annexe~\ref{appendix:ml}}.
...
\centering
\vspace{0.5cm}
\textbf{\large RÉSULTATS DU MODÈLE}\\[0.5cm]
\rule{0.9\textwidth}{0.5pt}\\[0.3cm]
Modèle utilisé : \textbf{Random Forest Classifier}\\[0.3cm]
Précision (Accuracy) : \textbf{\LARGE 87.34\%}\\[0.3cm]
...
\toprule
\textbf{Réel / Prédit} & \textbf{Lose (0)} & \textbf{Win (1)} \\
\midrule
...
\begin{itemize}
    \item Taux de Vrais Positifs (Recall) : $\frac{735}{735+81} = 90.07\%$
    \item Taux de Vrais Négatifs : $\frac{727}{727+131} = 84.73\%$
    \item Précision : $\frac{735}{735+131} = 84.87\%$
\end{itemize}

\subsection{Analyse de l'Importance des Features}

\begin{table}[H]
\centering
\caption{Classement de l'importance des features}
...
Ces résultats montrent que le \textbf{travail d'équipe} (\texttt{assists} avec 0.311) est le facteur le plus déterminant sur le résultat du match, suivi par la \textbf{survie} (\texttt{deaths} avec 0.205) et les \textbf{dégâts aux objectifs} (\texttt{tower\_damage} avec 0.147).
 
Contrairement aux attentes, les facteurs économiques (\texttt{gold\_per\_min} : 0.048, \texttt{xp\_per\_min} : 0.073) ont une importance relativement faible. Cela suggère que dans les matchs professionnels de Dota 2, la coordination d'équipe et le jeu orienté objectifs sont plus prédictifs de la victoire que l'accumulation individuelle de ressources.
