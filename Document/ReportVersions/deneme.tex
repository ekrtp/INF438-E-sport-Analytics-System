...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{Machine Learning :} Développement d'un modèle de prédiction des résultats de matchs avec optimisation des hyperparamètres et suivi MLflow
\end{enumerate}
...
esports\_gold.ml\_features & Features pour le ML & 8 311 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{04\_ml\_model.ipynb} : Entraînement du modèle ML avec hyperparameter tuning
\end{enumerate}
...
Toutes les expériences de modèle ont été automatiquement suivies sur MLflow avec \texttt{mlflow.autolog()}. Cette approche permet de :

\begin{itemize}
    \item Enregistrer automatiquement tous les paramètres testés
    \item Sauvegarder les métriques de chaque itération de cross-validation
    \item Stocker le meilleur modèle pour un déploiement futur
...
Hyperparameter Tuning & 9 combinaisons testées avec 5-Fold CV \\
Modèle ML & Random Forest avec 87.93\% d'accuracy \\
...
\begin{itemize}
    ...
    \item \textbf{Pratiques MLOps :} Importance du suivi des expériences avec MLflow et de l'optimisation systématique des hyperparamètres
    \item \textbf{Validation Croisée :} La 5-Fold CV permet une évaluation plus robuste que le simple train/test split
\end{itemize}

\subsection{Conclusion Générale}

Les services intégrés offerts par l'écosystème Azure ont permis le développement rapide et efficace d'une solution de données de bout en bout. Grâce à l'approche systématique de Grid Search avec Cross-Validation, nous avons pu identifier les paramètres optimaux (\texttt{numTrees=50}, \texttt{maxDepth=15}) pour notre modèle Random Forest, atteignant une accuracy de \textbf{87.93\%}.

L'analyse de l'importance des features a révélé que le \textbf{travail d'équipe} (\texttt{assists} : 0.2806) et la \textbf{survie} (\texttt{deaths} : 0.1953) sont les facteurs les plus déterminants pour prédire la victoire, tandis que les métriques économiques individuelles ont un impact plus limité. Ces insights sont cohérents avec la nature compétitive et stratégique de Dota 2 au niveau professionnel.
...
\newpage
\subsection*{Code Source du Modèle ML (avec Hyperparameter Tuning)}
\label{appendix:ml}

\begin{lstlisting}[language=Python, caption=Pipeline Machine Learning avec Grid Search et Cross-Validation]
import mlflow
import pandas as pd
...
storage_account = "dota2lakehousenew"
container = "data"
access_key = "***" # Cle masquee pour securite
...
# Grid Search - Definition de la grille de parametres
...
print(f"Meilleurs parametres: numTrees={bestRF.getNumTrees}, maxDepth={bestRF.getMaxDepth()}")
...
Processing & Apache Spark & 3.4.1 / 4.0.0 \\
Langage & Python / PySpark & 3.10 / 3.12 \\
Format de Données & Delta Lake & 2.4.0 \\
ML Tracking & MLflow & Auto-enabled \\
ML Tuning & Spark MLlib & CrossValidator \\
