...
A binary classification model was developed to predict the outcome of a professional Dota 2 match. The target variable is defined as \texttt{win} $\in \{0, 1\}$, where 1 represents a victory and 0 a defeat.
...
\begin{table}[H]
\centering
\caption{Explanatory variables used for modeling}
\label{tab:features}
\begin{tabular}{>{\centering\arraybackslash}m{3.5cm}>{\centering\arraybackslash}m{5cm}>{\centering\arraybackslash}m{3cm}}
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Category} \\
\midrule
kills & Number of eliminations performed & Combat \\
deaths & Number of deaths suffered & Combat \\
assists & Number of assists & Combat \\
gold\_per\_min & Gold accumulated per minute & Economy \\
xp\_per\_min & Experience gained per minute & Economy \\
hero\_damage & Damage dealt to enemy heroes & Damage \\
tower\_damage & Damage dealt to structures & Damage \\
last\_hits & Number of last hits on creeps & Progression \\
level & Level reached at end of game & Progression \\
duration\_minutes & Total match duration & Contextual \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Hyperparameter Optimization Methodology}

The \textbf{Random Forest Classifier} algorithm was selected for this classification task due to its robustness against overfitting and its ability to handle heterogeneous data without requiring prior normalization.

To identify the optimal hyperparameter configuration, an exhaustive \textbf{Grid Search} approach combined with \textbf{stratified 5-fold cross-validation (5-Fold CV)} was implemented. This methodology enables objective evaluation of the model's generalization capability while minimizing the risk of overfitting.
...
\toprule
\textbf{Hyperparameter} & \textbf{Values Tested} & \textbf{Cardinality} \\
\midrule
...
Table~\ref{tab:tuning-results} presents the average performance obtained for each hyperparameter combination, ordered by decreasing accuracy.

\begin{table}[H]
\centering
\caption{Comparative performance of tested configurations (CV Accuracy)}
...
\toprule
\textbf{numTrees} & \textbf{maxDepth} & \textbf{CV Accuracy (\%)} & \textbf{Rank} \\
\midrule
...
\begin{itemize}
    \item \textbf{Impact of depth:} The \texttt{maxDepth} parameter exerts a predominant influence on performance. Models configured with \texttt{maxDepth=15} consistently outperform those limited to \texttt{maxDepth=5}, with an average gap of $\approx$2.8 percentage points.
    
    \item \textbf{Impact of number of trees:} The \texttt{numTrees} parameter has a marginal effect on performance. At equal depth, the accuracy variation between 30 and 100 trees remains below 0.5\%, suggesting a performance plateau.
    
    \item \textbf{Optimal configuration:} The combination \texttt{numTrees=50, maxDepth=15} maximizes accuracy (88.70\%) while maintaining reasonable computational complexity.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Configuration selected for the final model}
...
\subsubsection{Global Performance Metrics}
...
\centering
\vspace{0.5cm}
\textbf{\large PERFORMANCE SUMMARY}\\[0.5cm]
...
\begin{table}[H]
\centering
\caption{Model confusion matrix on test set ($n=1674$)}
\label{tab:confusion-matrix}
\begin{tabular}{>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}>{\centering\arraybackslash}m{3cm}}
\toprule
\textbf{Actual $\backslash$ Predicted} & \textbf{Loss (0)} & \textbf{Win (1)} \\
\midrule
\textbf{Loss (0)} & 741 (TN) & 117 (FP) \\
\textbf{Win (1)} & 85 (FN) & 731 (TP) \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Detailed Metrics Calculation}

From the confusion matrix, evaluation metrics are calculated as follows:

\begin{align}
\text{Accuracy} &= \frac{TP + TN}{TP + TN + FP + FN} = \frac{731 + 741}{1674} = \frac{1472}{1674} = 87.93\% \\[0.3cm]
\text{Precision} &= \frac{TP}{TP + FP} = \frac{731}{731 + 117} = \frac{731}{848} = 86.20\% \\[0.3cm]
\text{Recall (Sensitivity)} &= \frac{TP}{TP + FN} = \frac{731}{731 + 85} = \frac{731}{816} = 89.58\% \\[0.3cm]
\text{Specificity} &= \frac{TN}{TN + FP} = \frac{741}{741 + 117} = \frac{741}{858} = 86.36\% \\[0.3cm]
...
The Random Forest algorithm enables estimation of the relative importance of each variable via the \textit{Mean Decrease in Impurity} (MDI) measure. Table~\ref{tab:feature-importance} presents the variable ranking in decreasing order of importance.

\begin{table}[H]
\centering
\caption{Variable importance in the optimized model (MDI)}
...
\toprule
\textbf{Rank} & \textbf{Variable} & \textbf{Importance} & \textbf{Cumulative (\%)} & \textbf{Category} \\
\midrule
1 & assists & 0.2806 & 28.06 & Combat \\
2 & deaths & 0.1953 & 47.59 & Combat \\
3 & tower\_damage & 0.1390 & 61.49 & Objectives \\
4 & xp\_per\_min & 0.0793 & 69.42 & Economy \\
5 & last\_hits & 0.0730 & 76.72 & Progression \\
6 & duration\_minutes & 0.0538 & 82.10 & Contextual \\
7 & kills & 0.0517 & 87.27 & Combat \\
8 & hero\_damage & 0.0516 & 92.43 & Damage \\
9 & gold\_per\_min & 0.0465 & 97.08 & Economy \\
...
\begin{enumerate}
    \item \textbf{Team play predominance:} The \texttt{assists} variable (28.06\%) emerges as the most influential predictor, highlighting that coordination and team synergy constitute more determining factors than isolated individual performances.
    
    \item \textbf{Importance of survival:} The \texttt{deaths} variable (19.53\%) occupies second position. This result suggests that minimizing deaths --- and by extension, maintaining economic advantage and map presence --- plays a crucial role in match outcome.
    
    \item \textbf{Objective orientation:} The \texttt{tower\_damage} variable (13.90\%) ranks third, confirming that structure destruction constitutes a key indicator of progression toward victory.
    
    \item \textbf{Secondary role of economy:} Contrary to initial hypotheses, economic metrics present moderate relative importance (\texttt{gold\_per\_min}: 4.65\%, \texttt{xp\_per\_min}: 7.93\%). This observation suggests that in the professional context, strategic use of resources takes precedence over their mere accumulation.
    
    \item \textbf{Limited contribution of eliminations:} The \texttt{kills} variable (5.17\%) displays surprisingly low importance, indicating that eliminations alone do not guarantee victory without conversion into tangible objective advantages.
\end{enumerate}
...
All model experiments were automatically tracked on MLflow using \texttt{mlflow.autolog()}. This approach enables:

\begin{itemize}
    \item Automatic recording of all tested parameters
    \item Saving metrics from each cross-validation iteration
    \item Storing the best model for future deployment
    \item Ensuring experiment reproducibility
\end{itemize}
...
\textbf{Problem:} Due to limitations of the \textit{Azure for Students} subscription, access to creating or selecting a Databricks cluster was restricted, which prevented notebook execution and thus launching SQL analyses on Gold data.

\textbf{Solution:} To ensure project progress, temporary access to a group member's Databricks cluster (\textit{esports-cluster}) was provided. This enabled SQL queries on Gold tables and continuation of the analytical phase. Access information was not exposed in plain text: the key and connection parameters were managed securely via Databricks environment variables and \textit{Databricks Secrets}.
...
\newpage
\subsection{ML Model Source Code (with Hyperparameter Tuning)}
\label{appendix:ml}

\begin{lstlisting}[language=Python, caption=Machine Learning pipeline with Grid Search and Cross-Validation]
import mlflow
import pandas as pd
...
storage_account = "dota2lakehousenew"
container = "data"
access_key = "***" # Key masked for security
...
# Grid Search - Parameter grid definition
...
# Display results
print(f"Best params: numTrees={bestRF.getNumTrees}, maxDepth={bestRF.getMaxDepth()}")
