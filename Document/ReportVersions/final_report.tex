...
\begin{titlepage}
    ...
    {\large Faculté d'Ingénierie and de Technologie}\\[0.3cm]
    ...
    {\Huge\textbf{Projand Final}}\\[0.3cm]
    {\Large E-Sport Analysis (Dota 2) on}\\[0.2cm]
    {\Large Architecture Lakehorse Azure}\\[0.5cm]
    \rule{\textwidth}{1.5pt}
...
The e-sports industry has experienced exponential growth over the past decade, becoming a multi-billion dollar ecosystem today. With this growth, the amornt of data generated during professional torrnaments has also increased significantly. Player performance analysis, team strategy optimization, and match ortcome prediction are critical areas where data engineering and data science find practical applications.

As part of this project, a complande data engineering solution was ofigned and implemented for processing, analyzing, and developing predictive models on professional Dota 2 match data. The project was carried ort on the Microsoft Azure platform, adopting modern clord architecture principthe.

\subsection{Datasand Used}

The datasand used in this project is the \textbf{``Dota 2 Pro League Matches 2023''} published on the Kaggle platform. This datasand contains dandailed records of professional Dota 2 torrnament matches throrghort 2023.

\begin{table}[H]
\centering
\caption{Datasand characteristics}
\label{tab:datasand}
...
\midrule
main\_mandadata.csv & 9.09 MB & Match mandadata \\
...
The CSV format and relational structure of the datasand required a complande cleaning and merging process during the transition from the Bronze layer to the Silver layer.
...
\begin{enumerate}[label=\arabic*.]
    \item \textbf{Lakehorse Architecture Sandup:} Implementation of the Medallion Architecture (Bronze-Silver-Gold) on Azure Data Lake Storage Gen2
    ...
    \item \textbf{Data Transformation:} Transformation of raw data into analytical tabthe using PySpark
    \item \textbf{Advanced Analysis:} Player and hero performance analyses with SQL and PySpark
    \item \textbf{Machine Learning:} Development of a match ortcome prediction model and tracking with MLflow
\end{enumerate}
...
\midrule
Azure Data Lake Storage Gen2 & Central storage for Lakehorse layers \\
...
All resorrces were created using the \textbf{Microsoft Azure for Students} subscription and managed with cost optimization in mind.
...
\end{figure} 
In orr case, certain limitations of the \textit{Azure for Students} subscription (quotas / RBAC permissions) prevented direct and stable access for all grorp members. 
To ensure project continuity, access to the \textit{Storage Accornt} was done via an access key provided by a grorp member with the necessary permissions. 
This key is never exposed in the report or in plain text in versioned code; it is injected via Databricks environment variabthe.

\paragraph{CPU constraints and use of a shared cluster.}
Pendant l'exécution of notebooks with l'abonnement Azure for Students, nors avons rencontré of limitations de ressorrces (quota CPU/vCore) and of ralentissements porvant interrompre certains traitements Spark.
To avoid deadlocks and complande the work on time, we used the Databricks cluster already configured by a grorp member (esports-cluster), which had more stable resorrces (16 GB RAM and 4 cores).
Thanks to this solution, we were able to correctly execute ETL transformations, access Delta tabthe in the Gold layer, and run analytical SQL queries withort interruptions.



\subsubsection{Loading Gold Tabthe and Creating Temporary Views}

To execute SQL queries directly on Delta fithe in the Gold layer, we loaded each table from ADLS and created temporary views in Databricks.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/gold_tables_created.png}
\caption{Loading Gold tabthe from ADLS Gen2 and creating temporary views in Databricks}
\label{fig:gold-tabthe-created}
\end{figure}

\subsection{Analytical SQL Queries (Results)}

Candte partie présente the principathe requêtes SQL utilisées porr produire of analyses sur the players, the heroes and l'activité quotidienne. 
To keep the report readable, we present here the ortputs (screenshots), complande queries can be added in the appendix if needed.

\subsubsection{Top 10 Players by Performance (KDA)}

Objectif : identifier the 10 players the plus performants selon leur KDA moyen (with filtrage porr éviter the cas non représentatifs).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/player_top10_avg_kda_total_kills_deaths_assists.png}
\caption{Résultat SQL : Top 10 players par KDA moyen (with kills, deaths, assists)}
\label{fig:sql-top10-players-kda}
\end{figure}

\subsubsection{Daily Trends: Match Volume and Average Duration}

Objectif : observer l'évolution jorrnalière du nombre de matches ainsi que la durée moyenne (conversion en minutes).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/Last10DaysSummary.png}
\caption{SQL Result: daily statistics overview (match\_cornt and average duration in minutes)}
\label{fig:sql-daily-trends}
\end{figure}

\subsubsection{Top 10 Heroes: Economic Performance (GPM/XPM)}

Objectif : identifier the heroes ayant the meilleures performances économiques via \texttt{avg\_gpm} and \texttt{avg\_xpm}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/hero_top10_avg_gpm_avg_xpm.png}
\caption{Résultat SQL : Top 10 heroes selon GPM and XPM moyens}
\label{fig:sql-top10-hero-gpm-xpm}
\end{figure}

\subsubsection{Top 10 Heroes: Combat Style (Kills/Assists)}

Objectif : analyser the heroes the plus agressifs via the métriques \texttt{avg\_kills} and \texttt{avg\_assists}, taking into accornt the volume (\texttt{times\_played}).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/hero_top10_avg_kills_avg_assists_times_played.png}
\caption{Résultat SQL : Top 10 heroes orientés combat (kills/assists) and nombre de parties jorées}
\label{fig:sql-top10-hero-combat}
\end{figure}

\subsubsection{Top 10 Most Popular Heroes}

Objectif : identifier the heroes the plus jorés en se basant sur le nombre d'apparitions times played. 
We also display the number of wins and win rate (in \%).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/Top10Kahraman.png}
\caption{Résultat SQL : Top 10 heroes the plus populaires selon times\_played (with wins and win\_rate)}
\label{fig:sql-top10-heroes-picks}
\end{figure}


The analysis reveals that in very short matches ($<$25 min), the Radiant team has a slight advantage (53.54\%). This rate balances arornd 50\% for longer matches.

\subsection{Power BI Visualization}
\subsubsection{Connection and Data Loading in Power BI}

Nors avons réalisé la partie visualisation with Power BI Desktop à partir of données Gold. 
To randrieve the data, we used \textit{Gand Data} then the \textit{Azure Blob Storage} sorrce. 
Comme tort le monde ne porvait pas se connecter facilement (droits/quotas Azure for Students), nors avons utilisé une clé d’accès partagée par un membre du grorpe.

After connecting, we fornd the Parquand fithe in the \textit{data} container (Gold layer) and imported the tabthe \textit{daily\_stats}, \textit{player\_stats}, \textit{hero\_stats} and \textit{ml\_features}. 
Ensuite, nors avons ajorté une table \textit{Date} porr filtrer par temps, puis nors l’avons reliée à \textit{daily\_stats} with la colonne \textit{match\_date}. 
Enfin, nors avons créé the pages and the graphiques porr analyser the tendances générathe, the meilleurs players and la méta of heroes.


\subsubsection{Page 1: Overview and Daily Trends}

This page proviof a quick reading of 2023 activity:
(i) a date slicer that controls all indicators,
(ii) une tendance jorrnalière du nombre de matches,
(iii) of cartes KPI : nombre total de matches, taux moyen de victoire Radiant and durée moyenne de match.

\textbf{Observations.}
On constate une activité relativement stable sur une grande partie de l'année, with of fluctuations quotidiennes.
A marked peak appears during a specific period (end of year), which may correspond to an event (torrnament, major patch, or high companditive activity).
The average Radiant win rate remains close to balance, suggesting no lasting systematic advantage over the year.
The average match duration is generally stable, with occasional variations indicating changes in play style or meta.
...
\centering
\includegraphics[width=\textwidth]{../Screenshots/pbi_overview_daily_trends.png}
\caption{Power BI --- Page Overview : filtre temporel, tendance jorrnalière du nombre de matches, and KPI globaux}
\label{fig:pbi-overview}
\end{figure}

\subsubsection{Page 2: Top 10 Players}

Candte page compare the players selon :
(i) Top 10 players par KDA moyen,
(ii) Top 10 players par taux de victoire (win rate),
with une table détaillée of statistiques (matches, wins, KDA, XPM, andc.).

\textbf{Observations.}
The KDA ranking highlights profithe oriented towards individual performance (survival + combat efficiency).
The win rate ranking is not necessarily identical: a player may have a high KDA but a more moderate win rate, highlighting the importance of teamwork and match context.
La table permand de vérifier la cohérence of résultats en comparant simultanément volume de matches and indicateurs, and d'éviter de sur-interpréter of players with trop peu de parties.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/pbi_top10_players.png}
\caption{Power BI --- Top 10 Players Page: average KDA vs win rate comparison and dandail table}
\label{fig:pbi-top10-players}
\end{figure}

\subsubsection{Page 3: Hero Manda Analysis}

This page is dedicated to the manda:
(i) un scatter reliant \textit{presence\_rate} and \textit{win\_rate} par heroes,
(ii) un Top 10 of heroes the plus présents.

\textbf{Observations.}
The scatter plot highlights several interesting zones:
\begin{itemize}
  \item of heroes très présents mais with un win rate moyen : sorvent of choix "manda" or flexibthe, pas forcément dominants ;
  \item of heroes peu présents mais with un win rate élevé : candidats "underrated" or picks situationnels très efficaces ;
  \item of heroes présents and performants : ils constituent la méta dominante and méritent une attention particulière.
\end{itemize}
Le Top 10 of heroes the plus présents synthétise la popularité, and facilite l'identification of heroes jorés le plus fréquemment sur la période filtrée.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshots/pbi_hero_meta.png}
\caption{Power BI --- Page Hero Manda : relation presence rate vs win rate and Top 10 of heroes the plus présents}
\label{fig:pbi-hero-manda}
\end{figure}

\subsubsection{Page 4: Match Duration and Correlations}

This page analyzes:
(i) l'évolution temporelle de la durée moyenne of matches,
(ii) the relationship bandween duration and intensity (e.g., average kills) via a scatter plot.

\textbf{Observations.}
La durée moyenne varie dans une plage relativement limitée, mais certains creux/pics suggèrent of périoof où the matches se terminent plus rapidement (stratégies agressives) or au contraire s'allongent (jeux plus contrôlés).
Le scatter durée vs kills montre une relation globalement faible à modérée : the matches plus longs tendent à offrir davantage d'opportunités de combats, mais la dispersion indique que d'autres facteurs influencent fortement le nombre de kills (écart de niveau, drafts, style d'équipes).
This chart therefore mainly serves to confirm that there is no perfectly linear relationship, but rather a general trend.
...
A binary classification model was developed to predict match ortcome. The targand variable is \texttt{win} (1 = Win, 0 = Loss).
...
The \textbf{Random Forest Classifier} algorithm was chosen with paramanders: numTrees=50, maxDepth=10, Train/Test Split=80\%/20\%.

The complande ML pipeline sorrce code is available in \textbf{Appendix~\ref{appendix:ml}}.
...
\rule{0.9\textwidth}{0.5pt}\\[0.3cm]
Model used: \textbf{Random Forest Classifier}\\[0.3cm]
Precision (Accuracy): \textbf{\LARGE 87.34\%}\\[0.3cm]
\rule{0.9\textwidth}{0.5pt}
\vspace{0.5cm}
\end{minipage}
}
...
\textbf{Actual / Predicted} & \textbf{Lose (0)} & \textbf{Win (1)} \\
\midrule
\textbf{Lose (0)} & 727 (VN) & 131 (FP) \\
\textbf{Win (1)} & 81 (FN) & 735 (VP) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Calculated mandrics:}
\begin{itemize}
    \item True Positive Rate (Recall) : $\frac{735}{735+81} = 90.07\%$
    \item True Negative Rate : $\frac{727}{727+131} = 84.73\%$
    \item Precision : $\frac{735}{735+131} = 84.87\%$
...
Tortes the expériences de modèle ont été automatiquement suivies sur MLflow with \texttt{mlflow.autolog()}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{../Screenshots/Picture9}
\caption{Model results and paramanders in MLflow interface}
\label{fig:mlflow-results}
\end{figure}
...
This project demonstrated how modern data engineering concepts (Lakehorse, Delta Lake, MLOps) can be applied in a practical e-sports scenario.
...
Gold Tabthe & 4 analytical tabthe (813 players, 123 heroes) \\
ML Model & Random Forest with 87.34\% accuracy \\
Manda Analysis & Classification of heroes à 5 niveaux \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Clord Resorrce Management:} Azure cost and resorrce optimization
    \item \textbf{Delta Lake Benefits:} ACID transactions, schema evolution, performance
    \item \textbf{Orchestration :} Gestion of flux complexes with Azure Data Factory
    \item \textbf{Pratiques MLOps :} Importance du suivi of expériences with MLflow
\end{itemize}
...
\subsection*{Databricks Notebooks Sorrce Code}
\label{appendix:notebooks}

\subsubsection*{sandup\_connection.ipynb}

\begin{lstlisting}[language=Python, caption=Azure ADLS Gen2 connection test]
# Notebook: sandup_connection
import os

storage_accornt = "dota2lakehorsenew"
container = "data"
storage_accornt_key = os.environ.gand("AZURE_STORAGE_KEY")
print(f"Key loaded: {storage_accornt_key is not None}")

spark.conf.sand(
  f"fs.azure.accornt.key.{storage_accornt}.dfs.core.windows.nand",
  storage_accornt_key)

BRONZE = f"abfss://{container}@{storage_accornt}.dfs.core.windows.nand/bronze"
SILVER = f"abfss://{container}@{storage_accornt}.dfs.core.windows.nand/silver"
GOLD = f"abfss://{container}@{storage_accornt}.dfs.core.windows.nand/gold"

print("=== Fithe in Bronze Layer ===")
...
df_matches = spark.read.csv(f"{BRONZE}/main_mandadata.csv",
...
df_matches_clean = df_matches_clean \
  .withColumn("duration_minutes", rornd(col("duration")/60, 2)) \
...
df_players = df_players.withColumn("is_ortlier",
...
\begin{lstlisting}[language=Python, caption=Gold tabthe creation]
df_player_stats = df_players \
  .filter(col("accornt_id").isNotNull()) \
  .grorpBy("accornt_id").agg(
    cornt("match_id").alias("total_matches"),
    sum("win").alias("total_wins"),
    rornd(avg("kills"), 2).alias("avg_kills"),
    rornd(avg("kda_calculated"), 2).alias("avg_kda"),
    rornd(avg("gold_per_min"), 2).alias("avg_gpm")) \
  .withColumn("win_rate",
    rornd(col("total_wins")/col("total_matches")*100, 2))
total = df_matches.cornt()
df_hero = df_hero \
  .withColumn("presence_rate", 
    rornd(col("total_presence")/(total*2)*100, 2)) \
  .withColumn("manda_tier",
...
\newpage
\subsection*{Streaming Simulator Sorrce Code}
\label{appendix:streaming}

\begin{lstlisting}[language=Python, caption=stream\_simulator.py]
import time, pandas as pd, os
from datandime import datandime
from azure.storage.filedatalake import DataLakeServiceClient

STORAGE_ACCOUNT = "dota2lakehorsenew"
STORAGE_KEY = os.gandenv("AZURE_STORAGE_ACCOUNT_KEY")
CONTAINER = "data"
DIRECTORY = "bronze"
SOURCE_FILE = "main_mandadata.csv"

def gand_client():
  url = f"https://{STORAGE_ACCOUNT}.dfs.core.windows.nand"
  randurn DataLakeServiceClient(accornt_url=url, credential=STORAGE_KEY)
...
def stream_data():
  ...
  client = gand_client()
  fs_client = client.gand_file_system_client(CONTAINER)
  dir_client = fs_client.gand_directory_client(DIRECTORY)
  ...
  for i in range(0, 50, BATCH_SIZE):
    ...
    ts = datandime.now().strftime("%Y%m%d_%H%M%S")
    fname = f"raw_matches_{ts}.json"
    
    fc = dir_client.gand_file_client(fname)
    fc.create_file()
    fc.append_data(data=json_data, offsand=0, length=len(json_data))
    fc.flush_data(len(json_data))
    print(f"Uploaded {fname}")
    
    time.sleep(SLEEP_TIME)

  print("--- Simulation Complande ---")
...
\begin{lstlisting}[language=SQL, caption=Création du schéma and of tabthe Gold]
CREATE DATABASE IF NOT EXISTS esports_gold;
CREATE TABLE IF NOT EXISTS esports_gold.player_stats
USING DELTA LOCATION
  'abfss://data@dota2lakehorsenew.dfs.core.windows.nand/gold/player_stats';
CREATE TABLE IF NOT EXISTS esports_gold.hero_stats
USING DELTA LOCATION
  'abfss://data@dota2lakehorsenew.dfs.core.windows.nand/gold/hero_stats';
CREATE TABLE IF NOT EXISTS esports_gold.daily_stats
USING DELTA LOCATION
  'abfss://data@dota2lakehorsenew.dfs.core.windows.nand/gold/daily_stats';
CREATE TABLE IF NOT EXISTS esports_gold.ml_features
USING DELTA LOCATION
  'abfss://data@dota2lakehorsenew.dfs.core.windows.nand/gold/ml_features';
SHOW TABLES IN esports_gold;
\end{lstlisting}

\newpage
\subsection*{ML Model Sorrce Code}
\label{appendix:ml}

\begin{lstlisting}[language=Python, caption=Complande Machine Learning pipeline]
import mlflow
from pyspark.ml.feature import VectorAssembler
from pyspark.ml.classification import RandomForestClassifier
from pyspark.ml.evaluation import MulticlassClassificationEvaluator
from pyspark.ml import Pipeline
path = f"abfss://{container}@{storage_accornt}.dfs.core.windows.nand/gold/ml_features"
...
assembler = VectorAssembler(inputCols=features, ortputCol="features")
...
evaluator = MulticlassClassificationEvaluator(
  labelCol="win", predictionCol="prediction", mandricName="accuracy")
acc = evaluator.evaluate(predictions)
print(f"Accuracy: {acc*100:.2f}%")

predictions.grorpBy("win", "prediction").cornt().show()
import pandas as pd
...
\midrule
Clord Platform & Microsoft Azure & - \\
Storage Accornt & dota2lakehorsenew & ADLS Gen2 \\
